...
class ParserTests(unittest.TestCase):
    ...
    def test_sectionLine1(self):
        self.assertListEqual(
            ...
            self.createEmptyParser().parseSectionLine('[F]faa [A]aaa  [D]dd d').measures
        )

    def test_sectionLine2(self):
        self.assertListEqual(
            [ Measure('jedna ', ''), Measure('dvě ', 'A'), Measure('Honza jde', 'D')],
            self.createEmptyParser().parseSectionLine('jedna [A]dvě [D]Honza jde').measures
        )

    def test_sectionLine3(self):
        self.assertListEqual(
            [ Measure('dvě ', 'A'), Measure('Honza jde', 'D'), Measure('', 'Hmol7')],
            self.createEmptyParser().parseSectionLine('[A]dvě [D]Honza jde[Hmol7]').measures
        )

    def test_sectionLine4(self):
        self.assertListEqual(
            [ Measure('a ', 'H9+'), Measure('b ', 'Fmi7+'), Measure('', 'Hmol7/G')],
            self.createEmptyParser().parseSectionLine('[H9+]a [Fmi7+]b [Hmol7/G]').measures
        )

    def test_sectionLine5(self):
        self.assertListEqual(
            [ Measure('Každej tu', 'Ab'), Measure('černou káru', 'G#'), Measure('svou', 'Db')],
            self.createEmptyParser().parseSectionLine('[Ab]Každej tu[G#]černou káru[Db]svou').measures
        )      
    ...
    def test_sectionLine8(self):
        self.assertListEqual(
            ...
            self.createEmptyParser().parseSectionLine('[H9+]a []b [Hmol7/G]').measures
        )
